...
function scoreDice (dice) {
    ...
    if(dice_counts.includes(6)){ //[6,6,6,6,6,6]
        score = checkSixtuple(dice_map,score);
    }
    if(dice_counts.includes(5)){ //[6,6,6,6,6,5]
        score += checkQuintuple(dice_map,score);
    }
    if(dice_counts.includes(4)){ //[6,6,6,6,5,5]
        score += checkQuadruple(dice_map,score);
    }
    if(dice_counts.includes(3)){ //[6,6,6,5,5,5]
...
function checkTriple (dice_map,score) {
    dice_map.forEach(function(value, key){
        if(value == 3){
            score += key * 100;
        }
